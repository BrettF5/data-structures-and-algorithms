...
const typeNum = (arr) => {
  return arr.filter((numberFilter) => typeof numberFilter === 'number');
};
...
const containsAnd = (arr) => {
  return arr.filter((andFilter) => andFilter.includes('and'));
};
...
const oddValues = (arr) => {
  return arr.filter((numberFilter) => numberFilter % 2 !== 0);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter((notIncluded) => !forbiddenValues.includes(notIncluded));
};
