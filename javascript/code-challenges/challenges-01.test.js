...
const addOne = (arr) => {
  const output = []; // put the new values in here
...
const addExclamation = (arr) => {
  ...
  arr.forEach((item) => {
    const itemPlusExclamation = item + '!';
    output.push(itemPlusExclamation);
  })
  return output;
};
...
const allUpperCase = (arr) => {
  const upperCaseOutput = [];

  arr.forEach((caps) => {
    const strUpperCase = caps.toUpperCase();
    upperCaseOutput.push(strUpperCase);
  });
...
const speaker = (words, callback) => {
  ...
  words.forEach((newStr) => {
    const modifiedByCallback = callback(newStr);
    newSpeaker.push(modifiedByCallback);
  });
